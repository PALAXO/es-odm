...
class BulkArray extends Array {
    ...
    async save(useVersion = false) {
        ...
        for (let i = 0; i < this.length; i++) {
            ...
            if (!item) {
                ...
                continue;
            } else if (item.constructor._alias.includes(`*`) || item.constructor._alias.includes(`?`)) {
                throw Error(`Item at position ${i} has wildcard in alias '${item.constructor._alias}'!`);
        ...
        for (let i = 0; i < itemsToProcess.length; i++) {
            ...
            bulkBody.push({
                index: {
                    _index: item.constructor._alias,
                    _id: item._id,
    ...
    async delete(useVersion = false) {
        ...
        for (let i = 0; i < itemsToProcess.length; i++) {
            ...
            bulkBody.push({
                delete: {
                    _index: _.get(item, `constructor._alias`, ``),
                    _id: _.get(item, `_id`, ``),
                    if_primary_term: (useVersion) ? item._primary_term : void 0,
                    if_seq_no: (useVersion) ? item._seq_no : void 0
                }
    ...
    get status() {
        ...
        for (const item of this) {
            ...
            if (uuid && !this.__status[uuid]) {
                this.__status[uuid] = newItem(item._id, item.constructor._alias);
            }
        }
...
async function fetchVersions(myLog, itemsToProcess) {
    ...
    for (const item of itemsToProcess) {
        if (!item._primary_term || !item._seq_no) {
            headPromises.push(myLog.callEsApi(`getHead`, item.constructor._alias, item._id));
            toFind.push(item);
        }
    }
    ...
    for (let i = 0; i < toFind.length; i++) {
        if (toFind[i]._version !== heads[i].body._version) {
            throw Error(`For item with id '${toFind[i]._id}' in alias '${toFind[i].constructor._alias}', specified version '${toFind[i]._version}', another version '${heads[i].body._version}' was found!`);
        }
...
function action(instance, item, rejected = true, statusCode = void 0, message = void 0) {
    ...
    if (!status[uuid]) {
        status[uuid] = newItem(item._id, item.constructor._alias);
    }
