    ...
    async save(useVersion = false) {
        ...
        for (let i = 0; i < this.length; i++) {
            ...
            if (!item) {
                ...
                continue;
            } else if (useVersion && (!item._version || !item._primary_term || !item._seq_no)) {
                //TODO - fetch
                throw Error(`Item at index ${i} doesn't have version related properties specified!`);
            }
        ...
        for (let i = 0; i < result.body.items.length; i++) {
            ...
            this[i]._primary_term = result.body.items[i].index._primary_term;
            this[i]._seq_no = result.body.items[i].index._seq_no;
