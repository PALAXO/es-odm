...
class BulkArray extends Array {
     ...
     * @param bulkArrays {...BulkArray} Optional bulk arrays to merge
     * @returns {Object} ES format bulk status
     */
    esStatus(includeAll = false, ...bulkArrays) {
        const items = [];
        let errors = false;
        let count = 0;

        const array = [this, ...bulkArrays];
        for (const arrayItem of array) {
            const status = arrayItem.status;
    ...
    clear() {
        ...
        for (let i = this.length - 1; i >= 0; i--) {
            if (!this[i]) {
                this.splice(i, 1);
            } else if (status[this[i].__uuid].state === STATES.REJECTED) {
                this.__rejected.push(this[i]);
                this.splice(i, 1);
            } else if (status[this[i].__uuid].state === STATES.FINISHED) {
