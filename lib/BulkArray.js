...
async function fetchVersions(myLog, itemsToProcess) {
    ...
    for (const item of itemsToProcess) {
        if (!item._primary_term || !item._seq_no) {
            headPromises.push(myLog.callEsApi(`getHead`, item.constructor.alias, item._id));
            toFind.push(item);
        }
    }
    ...
    for (let i = 0; i < toFind.length; i++) {
        if (toFind[i]._version !== heads[i].body._version) {
            throw Error(`For item with id '${toFind[i]._id}' in alias '${toFind[i].constructor.alias}', specified version '${toFind[i]._version}', another version '${heads[i].body._version}' was found!`);
        }
