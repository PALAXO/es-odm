...
class BulkArray extends Array {
    ...
    async save(useVersion = false) {
        const myLog = logger.createLogger(esClient, `bulkSave`);
        ...
        if (useVersion) {
            await fetchVersions(myLog, itemsToProcess);
        }
        ...
        myLog.logApiResponse(`Records saved via bulk.`);
        return result.body;
    }
    ...
    async delete(useVersion = false) {
        const myLog = logger.createLogger(esClient, `bulkDelete`);
        ...
        myLog.logApiResponse(`Records deleted via bulk.`);
        return result.body;
    }
     ...
     */
    async reload() {
        const myLog = logger.createLogger(esClient, `reload`);
        const status = this.status;
        ...
        for (let i = 0; i < this.length; i++) {
            ...
            if (!item) {
                ...
                this.reject(item, 400, `Item is not saved in ES yet.`);
            }
        ...
        myLog.logApiRequest({}, `Reloading BulkArray items.`);
        const searchCache = {};
        for (const reloadingAlias of Object.keys(reloadCache)) {
            ...
            while (processed < toReload.items.length) {
                ...
                let myResults = await myLog.callEsApi(`mget`, toReload.Odm.alias, itemsToProcess.map((item) => item._id));
                ...
                for (let i = 0; i < itemsToProcess.length; i++) {
                    ...
                    if (foundResult.found !== true) {
                        ...
                        newInstances.push(new toReload.Odm(toReload.Odm._unpackData(foundResult._source), foundResult._id, foundResult._version, foundResult.highlight, foundResult._primary_term, foundResult._seq_no, 1));
                    }
                }
                ...
                for (let i = 0; i < hlpItemArray.length; i++) {
                    ...
                    //Set new properties
                    item._id = newInstance._id;
                    item._version = newInstance._version;
                    //highlight remains the same
                    item._primary_term = newInstance._primary_term;
                    item._seq_no = newInstance._seq_no;
                    //score remains the same

                    Object.assign(item, newInstance);
                }
            }
        }
        myLog.logApiResponse(`BulkArray items reloaded.`);
 ...
 * @returns {Promise<void>}
 */
async function fetchVersions(myLog, itemsToProcess) {
    const fetchCache = {};
    ...
    for (const fetchingAlias of Object.keys(fetchCache)) {
        ...
        while (processed < toFetch.items.length) {
            ...
            let myResults = await myLog.callEsApi(`mget`, toFetch.Odm.alias, itemsToFetch.map((item) => item._id), false);
            myResults = myResults.body.docs;
