...
module.exports = {
    ...
    async search(self, myAction, parseResult, returnArray, defaultScrollSize, body, from, size, source, explicitScroll, additional) {
        ...
        const usesPIT = _.has(body, `pit.id`);
        if (!isExplicitSize && usesPIT) {
            throw Error(`You have to specify size parameter in case of search with "Point In Time".`);
        }

        //Check if use scroll
        const isExplicitScroll = !_.isNil(explicitScroll);
        const useScroll = ((from + size) > configuration.MAX_RESULTS) || isExplicitScroll;
        if (useScroll && usesPIT) {
            throw Error(`You cannot use scroll along with "Point In Time" in search.`);
        }
        ...
        if (initScroll) {
            ...
            const myAlias = (usesPIT) ? void 0 : self.alias;
            esResults = await myAction.callEs(`search`, myAlias, body, myFrom, mySize, useScroll, source, myScrollTimeout);
        }
        ...
        if (esResults.body.hits?.total?.value) {
            Object.defineProperty(returnArray, `_total`, {
                value: esResults.body.hits.total.value,
                writable: true,
                enumerable: false
            });
        }
