...
const MAXIMUM_BODY_LENGTH = 8192; //2^13
            ...
            stream.stream = {
                write: (data) => {
                    ...
                    if (data?.msg) {
                        ...
                        const bodyLength = JSON.stringify(data.msg)?.length;
                        const isTooLong = (_.isNumber(bodyLength)) ? (bodyLength > MAXIMUM_BODY_LENGTH) : false;
                        if (isTooLong) {
                            data.msg = `***too long content***`;
                        }
                    }
