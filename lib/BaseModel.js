...
class BaseModel {
    ...
    static async search(body, from = void 0, size = void 0, scroll = void 0) {
        ...
        for (const result of results.body.hits.hits) {
            if (this.__typeInIndex) {
                ...
                bulk.push(new TypedClass(result._source, result._id));
            } else {
                bulk.push(new this(result._source, result._id));
            }
        }
        return bulk;
    }
    ...
    async validate() {
        const objectCopy = _.cloneDeep(this);
        return Joi.validate(objectCopy, objectCopy.constructor.__schema, defaultConfiguration.validatorConfig);
    }
