...
class BaseModel {
    ...
    static async search(body, from = void 0, size = void 0) {
        ...
        let results = await this.__es.search(this.__fullIndex, this.__esType, body, 0, mySize, (mySize > 0) ? true : false);
        if (results.body.aggregations) {
            Object.defineProperty(bulk, `aggregations`, {
                value: results.body.aggregations,
                writable: true,
                enumerable: false
            });
        }
        bulk._total = results.body.hits.total;
        ...
        do {
            ...
            for (let i = startIndex; i < results.body.hits.hits.length; i++) {
                ...
                bulk.push(new constructor(result._source, result._id, result._version, result._highlight));
    ...
    static async get(ids) {
        ...
        for (const myId of ids) {
            promises.push((async () => {
                ...
                return new this(result.body._source, result.body._id, result.body._version, result.body._highlight);
            })());
        }
