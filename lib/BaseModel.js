...
class BaseModel {
    ...
    static async getSettings(includeDefaults = false) {
        const myLog = logger.createLogger(esClient, `getSettings`, this.__fullIndex);
        myLog.logApiRequest({}, `Retrieving settings.`);

        const result = await myLog.callEsApi(`getSettings`, this.__fullIndex, includeDefaults);
    ...
    static async putSettings(settings) {
        checkModelValidity(this, `putSettings`);

        const myLog = logger.createLogger(esClient, `putSettings`, this.__fullIndex);
        myLog.logApiRequest({}, `Updating settings.`);

        const result = await myLog.callEsApi(`putSettings`, this.__fullIndex, settings);
