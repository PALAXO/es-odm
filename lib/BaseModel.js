...
class BaseModel {
    ...
    static async search(body, from = void 0, size = void 0, source = void 0) {
        ...
        const { result, time } = await logger.measure(esClient, `search`, this.__fullIndex, body, void 0, mySize, (mySize > 0), source);
        let results = result;
        totalTime += time;
        ...
        do {
            ...
            if (results.body.hits.hits.length > 0 && !full) {
                const { result, time } = await logger.measure(esClient, `scroll`, results.body._scroll_id);
                totalTime += time;
                totalSize += parseInt(_.get(result, `headers['content-length']`, `0`), 10);
                scrolls++;
                results = result;
