...
class BaseModel {
    ...
    static async search(body, from = void 0, size = void 0, source = void 0) {
        ...
        //Check if use scroll
        const useScroll = (from + size) > defaultConfiguration.MAX_RESULTS;
        const myFrom = (useScroll) ? 0 : from;  //For scroll, do not specify real from, always start from 0
        const mySize = useScroll ? defaultConfiguration.MAX_RESULTS : size;
        ...
        logger.info({
            ...
            parameters: {
                from: from, size: size, source: source, useScroll: useScroll
            },
            msg: `Searching for records.`
        });

        //Perform request
        const { result, time } = await logger.measure(esClient, `search`, this.__fullIndex, body, myFrom, mySize, useScroll, source);
        totalTime += time;
        ...
        do {
            ...
            for (let i = startIndex; i < esResults.body.hits.hits.length; i++) {
            ...
            if (useScroll && esResults.body.hits.hits.length > 0 && !full) {
        ...
        } while (useScroll && esResults.body.hits.hits.length > 0 && !full);
        ...
        //Clear scroll
        if (useScroll && !_.isEmpty(esResults.body._scroll_id)) {
            const scrollId = esResults.body._scroll_id;
            logger.debug({
                action: `clearScroll`,
                type: `apiRequest`,
                index: this.__fullIndex,
            ...
            const clearScroll = async function() {
                try {
