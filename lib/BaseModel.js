...
class BaseModel {
     ...
     * @param scroll {boolean} Should be scroll used?
     * @returns {Promise<Array<BaseModel> | Object>}
     */
    static async search(body, from = void 0, size = void 0, scroll = false) {
        if (_.isNil(body) || (!_.isObject(body) && !_.isString(body))) {
            throw Error(`Body must be an object or _scroll_id!`);
        }

        let results;
        if (_.isObject(body)) {
            results = await this.__es.search(this.__fullIndex, this.__esType, body, from, size, scroll);
        } else {
            results = await this.__es.scroll(body);
        }

        const bulk = new BulkArray();
        for (const result of results.body.hits.hits) {
            if (this.__typeInIndex) {
        ...
        if (!scroll) {
            ...
            return { bulk, scrollId: results.body._scroll_id };
        }
    }
    ...
    static async updateByQuery(body) {
        if (this.__typeInIndex && this._type === `*`) {
            throw Error(`You cannot use 'updateByQuery' with current type!`);
        }
    ...
    async validate() {
        const objectCopy = _.cloneDeep(this);
        return Joi.validate(objectCopy, objectCopy.constructor.__schema, defaultConfiguration.VALIDATOR_CONFIG);
    }
 ...
 * @param returnOriginal {boolean} Returns original BaseModel, nothing is copied, use with caution
 * @returns {BaseModel}
 */
function cloneClass(properties = {}, returnOriginal = false) {
    if (returnOriginal) {
        return BaseModel;
    }
