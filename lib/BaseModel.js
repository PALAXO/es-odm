...
class BaseModel {
    ...
    static async search(body, from = void 0, size = void 0, source = void 0, explicitScroll = void 0) {
        ...
        const allAliases = await myLog.callEsApi(`getIndicesFromAlias`, `*`);
        const aliasMap = {};
        for (const [key, value] of Object.entries(allAliases.body)) {
            aliasMap[key] = Object.keys(value.aliases)[0];  //Assume there is always only one alias for each index
        }
        ...
        do {
            ...
            for (let i = startIndex; (i < esResults.body.hits.hits.length) && (counter < from + size); i++) {
                ...
                if (_.isNil(source)) {  //Create instance
                    const newIndex = (aliasMap[result._index]) ? aliasMap[result._index] : result._index;

                    let Constructor = constructorCache[newIndex];
                    if (!Constructor) {
                        Constructor = this;

                        const parsedIndex = this._parseIndex(newIndex);
                        Constructor = Constructor.in(parsedIndex.tenant);
                        if (parsedIndex.type) {
                            Constructor = Constructor.type(parsedIndex.type);
                        }
    ...
    static async delete(ids, version = void 0) {
        ...
        const result = await myLog.callEsApi(`bulk`, bulkBody, this._immediateRefresh);
    ...
    static async updateByQuery(body) {
        ...
        const result = await myLog.callEsApi(`updateByQuery`, this._alias, body, this._immediateRefresh);
    ...
    static async deleteByQuery(body) {
        ...
        const result = await myLog.callEsApi(`deleteByQuery`, this._alias, body, this._immediateRefresh);
     ...
     * @param setAlias  {boolean}       Automatically set alias
    ...
    static async getIndex() {
        checkModelValidity(this, `getIndexFromAlias`);

        const myLog = logger.createLogger(esClient, `getIndexFromAlias`, this._alias);
    ...
    static async putAliasTo(index) {
        checkModelValidity(this, `setAlias`);

        const myLog = logger.createLogger(esClient, `putAlias`, this._alias);
    ...
    static async deleteAlias() {
        checkModelValidity(this, `deleteAliasFrom`);

        const myLog = logger.createLogger(esClient, `deleteAliasFrom`, this._alias);
    ...
    static async indexExists() {
        ...
        const myLog = logger.createLogger(esClient, `indexExists`, this._alias);
        myLog.logApiRequest({}, `Checking index existence.`);

        const result = await myLog.callEsApi(`indexExists`, this._alias);
     ...
     * @param destinationModel  {BaseModelType | string}    Destination type
     * @param script            {string}                    Script source
     * @returns                 {Promise<Object>}           ES response
     */
    static async reindex(destinationModel, script = void 0) {
        if (!destinationModel) {
        ...
        const result = await myLog.callEsApi(`reindex`, body, this._immediateRefresh);
    ...
    static async cloneIndex(settings = void 0) {
        ...
        let realIndex = this._alias;   //Legacy - for case when ES is not migrated to aliases yet
        const isAlias = await myLog.callEsApi(`existsAlias`, this._alias);
        if (isAlias.body === true) {
            const indices = await myLog.callEsApi(`getIndicesFromAlias`, this._alias);
            if (Object.keys(indices.body).length > 1) {
    ...
    static async getTypes() {
        if (_.isNil(this._name) || !_.isString(this._name) || this._name.includes(`*`)) {
            throw Error(`You cannot use 'getTypes' with current base name '${this._name}', full alias is '${this._alias}'!`);
        } else if (_.isEmpty(this._tenant) || this._tenant.includes(`*`)) {
        ...
        if (this.hasTypes()) {
            ...
            indices.map((index) => {
                const typeIndex = index.lastIndexOf(`_`) + 1;
                const type = index.substring(typeIndex);
...
function checkModelValidity(model, functionName) {
    if (_.isNil(model._name) || !_.isString(model._name) || model._name.includes(`*`)) {
    ...
    } else if (!_.isEmpty(model._type) && model._type.includes(`*`)) {
