...
class BaseModel {
     ...
     * @param additional        {{cache: Object}}               Additional data
    ...
    static async search(body, from = void 0, size = void 0, source = void 0, explicitScroll = void 0, additional = void 0) {
        ...
        if (initScroll) {
            ...
            //Normal search
            esResults = await myLog.callEsApi(`search`, this._alias, alteredBody, myFrom, mySize, useScroll, source, (isExplicitScroll) ? explicitScroll: void 0);
        }
    ...
    static async count(body = void 0) {
        ...
        const result = await myLog.callEsApi(`count`, this._alias, this._alterSearch(body));
    ...
    static async updateByQuery(body, scrollSize = void 0, waitForCompletion = true) {
        ...
        const result = await myLog.callEsApi(`updateByQuery`, this._alias, this._alterSearch(body), scrollSize, waitForCompletion, this._immediateRefresh);
    ...
    static async deleteByQuery(body, scrollSize = void 0, waitForCompletion = true) {
        ...
        const result = await myLog.callEsApi(`deleteByQuery`, this._alias, this._alterSearch(body), scrollSize, waitForCompletion, this._immediateRefresh);
    ...
    async save(useVersion = false) {
        ...
        const result = await myLog.callEsApi(`index`, this.constructor._alias, await this._packData(), this._id,
    ...
    static __checkIfFullySpecified(functionName) {
        if (_.isNil(this._name) || !_.isString(this._name) || this._name.includes(`*`) || this._name.includes(`?`)) {
            throw Error(`You cannot use '${functionName}' with current base name '${this._name}', full alias is '${this._alias}'!`);
        } else if (_.isEmpty(this._tenant) || this._tenant.includes(`*`) || this._tenant.includes(`?`)) {
            throw Error(`You cannot use '${functionName}' with current tenant '${this._tenant}', full alias is '${this._alias}'!`);
        } else if (!_.isEmpty(this._type) && (this._type.includes(`*`) || this._type.includes(`?`))) {
            throw Error(`You cannot use '${functionName}' with current type '${this._type}', full alias is '${this._alias}'!`);
        }
    }
     ...
     * @returns {BaseModel} Constructor to be used
     * @private
     */
    static __getConstructor(searchResult, constructorCache) {
        ...
        if (!Constructor) {
            ...
            if (!_.isEmpty(Constructor._type) && Constructor._type !== indexInfo.type) {
